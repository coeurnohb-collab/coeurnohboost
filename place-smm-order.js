...
const {
  initFirebaseAdmin, verifyCaller, enforceRateLimit, cleanText, isHttpsUrl
} = require('./_lib/security');

initFirebaseAdmin();
const db = admin.firestore();

// fetch avec delai maximum : si MoreThanPanel ne repond pas, on abandonne
// proprement (la commande part alors en "pending_review") au lieu de rester
// bloque jusqu'a la coupure de la fonction Vercel.
...
async function refundOrderSafely(userRef, orderRef, amount, reasonForClient) {
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      const adminSnap = userRef.id !== ADMIN_UID ? await tx.get(adminRef) : null;
      ...
      tx.set(db.collection('wallet_transactions').doc(), buildWalletTxEntry({
        uid: userRef.id, type: 'order_refund', amount, balanceAfter: newBalance,
        description: 'Remboursement — commande refusée', relatedId: orderRef.id
      }));

      // Reprise du revenu admin credite au moment de l'achat (voir
      // place-smm-order plus bas) : la commande est annulee, ce revenu
      // n'a plus lieu d'etre.
      if (userRef.id !== ADMIN_UID && adminSnap && adminSnap.exists) {
        const adminBalance = adminSnap.data().balance || 0;
        const newAdminBalance = Math.round((adminBalance - amount) * 100) / 100;
        tx.update(adminRef, { balance: newAdminBalance });
        tx.set(db.collection('wallet_transactions').doc(), buildWalletTxEntry({
          uid: ADMIN_UID, type: 'order_income_reversed', amount: -amount, balanceAfter: newAdminBalance,
          description: 'Commande refusée, revenu annulé', relatedId: orderRef.id
        }));
      }
...
module.exports = async function handler(req, res) {
  ...
  // 1) Identite obligatoire -- verifiee via security.js (verifyIdToken avec
  // controle de revocation : un jeton vole reste desormais invalide des que
  // "Se deconnecter de tous les appareils" a ete utilise, au lieu de rester
  // valable jusqu'a 1h apres).
  let uid, userEmail;
  try {
    const caller = await verifyCaller(req);
    uid = caller.uid;
    userEmail = caller.email;
    await enforceRateLimit({ scope: 'smm-order', id: uid, limit: 20, windowSec: 600 });
  } catch (e) {
    const status = e.status || 401;
    if (status === 429) res.setHeader('Retry-After', String((e.extra && e.extra.retryAfterSec) || 60));
    return res.status(status).json({ error: e.message || 'Session invalide, reconnecte-toi.' });
  }

  // 2) Validation de base des champs. Le lien doit etre une vraie URL https
  // (jamais du texte libre stocke tel quel : evite d'enregistrer un lien
  // interne malveillant ou un texte demesure dans la commande).
  if (!orderKind || !link || price == null) {
    return res.status(400).json({ error: 'Parametres manquants.' });
  }
  if (!isHttpsUrl(link, 500)) {
    return res.status(400).json({ error: 'Le lien doit être une URL valide (https://...).' });
  }
  ...
  try {
    await db.runTransaction(async (tx) => {
      ...
      // ---- PHASE LECTURE (avant toute ecriture) ----
      const adminRef = db.collection('users').doc(ADMIN_UID);
      const adminSnap = uid !== ADMIN_UID ? await tx.get(adminRef) : null;
      ...
      tx.set(orderRef, {
        uid,
        email: userEmail,
        platform: cleanText(platformName || platform || '', 80),
        service: cleanText(service || '', 200),
        quality: cleanText(String(quality || orderKind), 40),
        link,
      ...
      tx.set(db.collection('wallet_transactions').doc(), buildWalletTxEntry({
        uid, type: 'order_purchase', amount: -numericPrice, balanceAfter: newBalance,
        description: `Commande ${platformName || platform || ''} — ${service || ''}`.trim(), relatedId: orderRef.id
      }));

      // Revenu direct pour CoeurnohBoost : il n'y a pas de "vendeur" tiers
      // ici (le service est fourni par la plateforme elle-meme, achetee en
      // gros chez le fournisseur MTP en dehors de ce portefeuille), donc le
      // prix entier paye par le client est credite au solde admin.
      if (uid !== ADMIN_UID && adminSnap && adminSnap.exists) {
        const adminBalance = adminSnap.data().balance || 0;
        const newAdminBalance = Math.round((adminBalance + numericPrice) * 100) / 100;
        tx.update(adminRef, { balance: newAdminBalance });
        tx.set(db.collection('wallet_transactions').doc(), buildWalletTxEntry({
          uid: ADMIN_UID, type: 'order_income', amount: numericPrice, balanceAfter: newAdminBalance,
          description: `Commande ${platformName || platform || ''} — ${service || ''}`.trim(), relatedId: orderRef.id
        }));
      }
    });
  ...
  try {
    const response = await fetchWithTimeout(MTP_API_URL, {
      method: 'POST',
    ...
    try {
      const statusResponse = await fetchWithTimeout(MTP_API_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
