...
module.exports = async function handler(req, res) {
  ...
  try {
    const { idToken, pubId, pubIds } = req.body;

    // SECURISE : avant, ce fichier faisait confiance a un simple "uid" envoye
    // dans la requete, sans verifier que la personne qui appelle est bien
    // connectee sous cette identite -- n'importe qui, meme sans compte,
    // pouvait declencher un "achat" facturant le solde de quelqu'un d'autre.
    // Desormais, l'identite vient uniquement du jeton Firebase verifie ici.
    if (!idToken) {
    ...
    if (!pubId && !(Array.isArray(pubIds) && pubIds.length > 0)) {
      return res.status(400).json({ error: 'pubId ou pubIds sont requis' });
    }
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      for (const it of items) {
        ...
        if (it.sellerUid !== ADMIN_UID && sellerSnaps[it.sellerUid] && sellerSnaps[it.sellerUid].exists) {
          ...
          adminCommissionTotal += it.effectivePrice;
        }
      ...
      for (const sUid of Object.keys(sellerBalanceDeltas)) {
        ...
        const newSellerBalance = Math.round((sellerBalance + sellerBalanceDeltas[sUid]) * 100) / 100;
      ...
      adminCommissionTotal = Math.round(adminCommissionTotal * 100) / 100;
      if (uid !== ADMIN_UID && adminCommissionTotal > 0 && adminSnap && adminSnap.exists) {
        const adminBalance = adminSnap.data().balance || 0;
        const newAdminBalance = Math.round((adminBalance + adminCommissionTotal) * 100) / 100;
    ...
    // Acheteur : un seul recap, meme pour un panier de plusieurs articles.
    ...
    await sendPushNotification(uid, 'Achat confirmé ✅', buyerSummary);

    // Un push par vendeur concerne (regroupe si plusieurs de ses articles ont ete achetes)
