...
async function handleCinetpay(uid, body) {
  ...
  try {
    ...
    return { status: 500, json: { error: 'Configuration serveur CinetPay manquante ou invalide' } };
  }
  ...
  if (!cinetpayResponse.ok || data.code !== 200 || !data.payment_url) {
    ...
    return { status: 400, json: { error: (data.details && data.details.message) || 'Erreur CinetPay' } };
  }
