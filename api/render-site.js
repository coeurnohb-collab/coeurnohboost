...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .gallery img{width:100%;aspect-ratio:1;object-fit:cover;border-radius:12px;border:1px solid var(--border);cursor:zoom-in}
  ...
  .post-card img{width:100%;height:180px;object-fit:cover;display:block;cursor:zoom-in}
  ...
  .lightbox{position:fixed;inset:0;z-index:50;background:rgba(10,12,18,.94);display:none;align-items:center;justify-content:center;padding:24px;cursor:zoom-out}
  .lightbox.open{display:flex}
  .lightbox img{max-width:100%;max-height:100%;border-radius:8px;box-shadow:0 20px 60px rgba(0,0,0,.5)}
  .lightbox-close{position:absolute;top:16px;right:18px;background:rgba(255,255,255,.12);color:#fff;border:none;width:38px;height:38px;border-radius:50%;font-size:1.3rem;cursor:pointer;line-height:1}
</style>
</head>
<body>
<div class="header">
...
<div class="wrap">
  <div class="hero">
    ...
    <div class="hero-card">
      ...
      <div class="hero-actions">
        ${waLink ? `<a class="btn btn-wa" href="${escapeAttr(waLink)}" target="_blank" rel="noopener">WhatsApp <span aria-hidden="true">→</span></a>` : ''}
  ...
  <div class="section" id="site-sec-gallery">
    ...
    <div class="gallery">${gallery.map((g, i) => `<img src="${escapeAttr(g)}" alt="${escapeAttr(site.businessName || '')} ${i + 1}" loading="lazy" onclick="openSiteLightbox('${escapeAttr(g).replace(/'/g, "\\'")}')">`).join('')}</div>
  </div>` : ''}
  ...
  <div class="section" id="site-sec-blog">
    ...
    <article class="post-card" data-category="${escapeAttr(post.category || '')}">
      ${post.imageUrl ? `<img src="${escapeAttr(post.imageUrl)}" alt="" loading="lazy" onclick="openSiteLightbox('${escapeAttr(post.imageUrl).replace(/'/g, "\\'")}')">` : ''}
      <div class="post-body">
...
${(gallery.length || blogPosts.some(p => p.imageUrl)) ? `<div class="lightbox" id="site-lightbox" onclick="closeSiteLightbox()">
...
<script>
function openSiteLightbox(src){
  var box = document.getElementById('site-lightbox');
  document.getElementById('site-lightbox-img').src = src;
