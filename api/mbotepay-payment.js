...
// ICI, cote serveur, avec le meme montant qui sert a calculer le vrai
// montant facture en mobile money -- avant, le navigateur creait cette
// demande lui-meme avec un montant totalement libre. Un client pouvait
// payer 1$ en mobile money et enregistrer une demande a 1000$ ; comme le
// webhook ne verifiait jamais le montant reellement paye, il se faisait
// crediter 1000$ au lieu de 1$.
...
module.exports = async function handler(req, res) {
  ...
  try {
    const { idToken, amountUSD, countryCode, operatorName, phone, chargeCurrency } = req.body;

    // SECURISE : avant, ce fichier faisait confiance a un simple "uid"
    // envoye dans la requete -- n'importe qui pouvait declencher une
    // demande de recharge Mobile Money attribuee au compte de quelqu'un
    // d'autre. Desormais l'identite vient uniquement du jeton Firebase
    // verifie ici.
    if (!idToken) {
    ...
    // Cree la demande de recharge ICI, avec le MEME montant (amountUSD) qui
    // a servi a calculer le montant reellement facture en mobile money
    // ci-dessus -- le client ne peut plus la falsifier ensuite.
    let userEmail = null;
    try {
