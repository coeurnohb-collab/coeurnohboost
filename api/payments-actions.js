...
const SITE_DOMAIN_PRICE_YEAR = 15;      // en $, par an -- doit rester identique a SITE_DOMAIN_PRICE_YEAR dans script.js
...
async function purchaseSiteDomain(uid, body) {
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    const buyerBalance = buyerSnap.data().balance || 0;
    if (buyerBalance < SITE_DOMAIN_PRICE_YEAR) {
      throw new Error('Solde insuffisant. Recharge ton portefeuille pour demander ce domaine.');
    }

    // ---- PHASE ECRITURE ----
    const newBuyerBalance = Math.round((buyerBalance - SITE_DOMAIN_PRICE_YEAR) * 100) / 100;
    transaction.update(buyerRef, { balance: newBuyerBalance });
    transaction.set(reqRef, {
      uid, desiredDomain, status: 'pending', pricePaid: SITE_DOMAIN_PRICE_YEAR,
      createdAt: new Date().toISOString()
    });
    transaction.set(db.collection('wallet_transactions').doc(), buildWalletTxEntry({
      uid, type: 'site_domain_purchase', amount: -SITE_DOMAIN_PRICE_YEAR, balanceAfter: newBuyerBalance,
      description: `Demande d'achat de domaine (${desiredDomain})`, relatedId: null
    }));

    if (uid !== ADMIN_UID && adminSnap && adminSnap.exists) {
      const adminBalance = adminSnap.data().balance || 0;
      const newAdminBalance = Math.round((adminBalance + SITE_DOMAIN_PRICE_YEAR) * 100) / 100;
      transaction.update(adminRef, { balance: newAdminBalance });
      transaction.set(db.collection('wallet_transactions').doc(), buildWalletTxEntry({
        uid: ADMIN_UID, type: 'site_domain_purchase_income', amount: SITE_DOMAIN_PRICE_YEAR, balanceAfter: newAdminBalance,
