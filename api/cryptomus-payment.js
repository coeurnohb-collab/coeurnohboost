...
module.exports = async function handler(req, res) {
  ...
  try {
    const { idToken, amount, currency } = req.body;

    // SECURISE : avant, ce fichier faisait confiance a un simple "uid"
    // envoye dans la requete -- n'importe qui pouvait creer une facture
    // Cryptomus attribuee au compte de quelqu'un d'autre. Desormais
    // l'identite vient uniquement du jeton Firebase verifie ici.
    if (!idToken) {
    ...
    if (!amount || amount <= 0) {
      return res.status(400).json({ error: 'amount (positif) est requis' });
    }
