  ...
  try {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      transaction.set(db.collection('wallet_transactions').doc(), buildWalletTxEntry({
        uid, type: 'topup_crypto', amount: amountPaid, balanceAfter: newBalance,
        description: 'Recharge par crypto-monnaie', relatedId: orderId
      }));
    ...
    await sendPushNotification(uid, 'Solde rechargé 💰', `Ton compte a été crédité de ${amountPaid.toFixed(2)}$.`);
