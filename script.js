...
/* ================= PAIEMENT BOUTIQUE (par article) ================= */
let shopCheckoutItem = null; // { pubId, title, price, type }
let shopCheckoutCountry = null;
let shopCheckoutOperator = null;
let shopCheckoutMethod = 'mobile';

function buyShopItem(pubId, title, price, itemType) {
  if (!currentUser) { openAuth('register'); return; }
  shopCheckoutItem = { pubId, title, price, type: itemType || 'product' };
  shopCheckoutCountry = null;
  shopCheckoutOperator = null;
  shopCheckoutMethod = 'mobile';
  ...
  const modalHtml = `
    <div class="modal-overlay" id="shop-checkout-modal">
      <div class="modal">
        ...
        <h2>🛍️ ${title}</h2>
        <p class="sub">Montant à payer : <strong>${price.toFixed(2)}$</strong></p>

        <div class="pay-method-tabs" id="shop-pay-method-tabs"></div>

        <div id="shop-pay-panel-mobile">
          <div class="field">
            <label>Pays</label>
            <select id="shop-pay-country-select" class="select-input" onchange="onShopCountryChange()"></select>
          </div>
          <div class="op-grid" id="shop-pay-operators"></div>
          <div class="field">
            <label>Numéro de téléphone</label>
            <input type="tel" id="shop-pay-phone" class="text-input" placeholder="+243...">
          </div>
        </div>

        <div class="modal-error hidden" id="shop-checkout-error"></div>
        <div class="modal-loading hidden" id="shop-checkout-loading">⏳ Paiement en cours...</div>
        <div class="modal-loading hidden" id="shop-checkout-waiting">
          📲 Confirme le paiement sur ton téléphone (code secret Mobile Money). On attend la confirmation automatiquement...
        </div>
        <div class="hidden" id="shop-checkout-success">
          <div class="tutorial-emoji">✅</div>
          <p class="sub" style="text-align:center;font-weight:700;color:var(--green)">Paiement confirmé !</p>
          <div id="shop-checkout-download"></div>
        </div>

        <button class="btn btn-primary" style="width:100%;margin-top:14px" id="shop-checkout-submit" onclick="submitShopPayment()">Payer maintenant</button>
      </div>
    </div>`;
  document.body.insertAdjacentHTML('beforeend', modalHtml);
  renderShopMethodTabs();
  renderShopCountrySelect();
}

function closeShopCheckout() {
  if (shopOrderUnsubscribe) { shopOrderUnsubscribe(); shopOrderUnsubscribe = null; }
  const modal = document.getElementById('shop-checkout-modal');
  if (modal) modal.remove();
}

function renderShopMethodTabs() {
  const methods = [
    { id: 'mobile', label: 'Mobile Money', icon: '📱' },
    { id: 'crypto', label: 'Crypto', icon: '₿' }
  ];
  document.getElementById('shop-pay-method-tabs').innerHTML = methods.map(m => `
    <button class="${m.id === shopCheckoutMethod ? 'active' : ''}" onclick="selectShopMethod('${m.id}')">${m.icon} ${m.label}</button>
  `).join('');
  document.getElementById('shop-pay-panel-mobile').classList.toggle('hidden', shopCheckoutMethod !== 'mobile');
}

function selectShopMethod(m) {
  shopCheckoutMethod = m;
  renderShopMethodTabs();
}

function renderShopCountrySelect() {
  const sel = document.getElementById('shop-pay-country-select');
  sel.innerHTML = `<option value="">Choisis ton pays</option>` +
    COUNTRIES.map(c => `<option value="${c.code}">${c.flag} ${c.name}</option>`).join('');
}

function onShopCountryChange() {
  shopCheckoutCountry = document.getElementById('shop-pay-country-select').value || null;
  shopCheckoutOperator = null;
  renderShopOperators();
}

function renderShopOperators() {
  const country = COUNTRIES.find(c => c.code === shopCheckoutCountry);
  const el = document.getElementById('shop-pay-operators');
  if (!country) { el.innerHTML = ''; return; }
  el.innerHTML = country.ops.map(op => {
    const badge = getOperatorBadge(op);
    return `
    <div class="op-card${op === shopCheckoutOperator ? ' active' : ''}" onclick="selectShopOperator('${op.replace(/'/g, "\\'")}')">
      <div class="op-icon" style="background:${badge.bg};${badge.dark ? 'color:#111' : 'color:#fff'}">${badge.label}</div>
      <span class="op-name">${op}</span>
    </div>`;
  }).join('');
...
function selectShopOperator(op) {
  shopCheckoutOperator = op;
  renderShopOperators();
}
...
async function submitShopPayment() {
  ...
  if (shopCheckoutMethod === 'crypto') {
    errEl.textContent = "Le paiement crypto pour la boutique arrive très bientôt. Utilise Mobile Money en attendant.";
    errEl.classList.remove('hidden');
    return;
  }

  const phone = document.getElementById('shop-pay-phone').value.trim();
  if (!shopCheckoutCountry || !shopCheckoutOperator || !phone) {
  ...
  document.getElementById('shop-checkout-loading').classList.remove('hidden');

  try {
    // 1) Tentative MboтePay
    const mbotepayResponse = await fetch('/api/shop-mbotepay-payment', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        uid: currentUser.uid,
        email: currentUser.email,
        pubId: shopCheckoutItem.pubId,
        itemTitle: shopCheckoutItem.title,
        itemType: shopCheckoutItem.type,
        amountUSD: shopCheckoutItem.price,
        countryCode: shopCheckoutCountry,
        operatorName: shopCheckoutOperator,
        phone
      })
    });
    const mbotepayData = await mbotepayResponse.json();

    if (mbotepayData.supported && mbotepayData.success) {
      const orderRef = await db.collection('shop_orders').add({
        ...
        method: 'mobile',
        gateway: 'mbotepay',
        country: shopCheckoutCountry,
        operator: shopCheckoutOperator,
        phone,
        mbotepayReference: mbotepayData.reference,
        localAmount: mbotepayData.localAmount,
        localCurrency: mbotepayData.currency,
        status: 'pending_payment',
        createdAt: new Date().toISOString()
      });
      document.getElementById('shop-checkout-loading').classList.add('hidden');
      document.getElementById('shop-checkout-waiting').classList.remove('hidden');
      watchShopOrder(orderRef.id);
      return;
    }
    if (mbotepayData.supported && !mbotepayData.success) {
      throw new Error(mbotepayData.error || "Erreur MboтePay");
    }

    // 2) Repli CinetPay (Mali, Burkina Faso, Togo, Niger, Guinee)
    const cinetpayResponse = await fetch('/api/shop-cinetpay-payment', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        uid: currentUser.uid,
    ...
    if (cinetpayData.supported && !cinetpayData.success) {
      throw new Error(cinetpayData.error || "Erreur CinetPay");
    }

    // 3) Aucune passerelle ne couvre ce pays : repli WhatsApp
    document.getElementById('shop-checkout-loading').classList.add('hidden');
    closeShopCheckout();
    const message = encodeURIComponent(`Bonjour, je souhaite commander : ${shopCheckoutItem.title} (${shopCheckoutItem.price}$)`);
    window.open(`https://wa.me/243825001290?text=${message}`, '_blank');
...
let shopOrderUnsubscribe = null;
function watchShopOrder(orderId) {
  if (shopOrderUnsubscribe) shopOrderUnsubscribe();
  shopOrderUnsubscribe = db.collection('shop_orders').doc(orderId).onSnapshot(async (doc) => {
    const data = doc.data();
    if (!data) return;
    if (data.status === 'completed') {
      shopOrderUnsubscribe();
      document.getElementById('shop-checkout-waiting').classList.add('hidden');
      document.getElementById('shop-checkout-success').classList.remove('hidden');
      if (data.itemType === 'book') {
        try {
          const pubDoc = await db.collection('publications').doc(data.pubId).get();
        ...
        } catch (e) { /* pas grave, le client peut nous contacter */ }
      } else {
        document.getElementById('shop-checkout-download').innerHTML =
      ...
      const errEl = document.getElementById('shop-checkout-error');
      document.getElementById('shop-checkout-submit').classList.remove('hidden');
      errEl.textContent = "Le paiement a échoué. Réessaie.";
      errEl.classList.remove('hidden');
    }
  });
}
