  ...
  <div class="shop-card" id="shop-card-${item.id}">
    ...
    <div class="shop-card-body">
      ...
      <div class="shop-card-actions">
        ...
        <button class="btn btn-primary btn-sm" style="margin-left:auto" onclick="buyShopItem('${item.id}','${escapeForJs(item.title)}',${item.price})" data-i18n="shop_buy">Commander</button>
      </div>
...
function buyShopItem(pubId, title, price) {
  if (!currentUser) { openAuth('register'); return; }
  const message = encodeURIComponent(`Bonjour, je souhaite commander : ${title} (${price}$)`);
  const modalHtml = `
    <div class="modal-overlay" id="shop-buy-modal" onclick="if(event.target===this) this.remove()">
      <div class="modal">
        <button class="modal-close" onclick="document.getElementById('shop-buy-modal').remove()">×</button>
        <h2>🚀 Presque prêt !</h2>
        <p class="sub">Le paiement automatique pour la boutique arrive très bientôt. En attendant, commande directement via WhatsApp — on te répond vite.</p>
        <a class="btn btn-primary" style="width:100%;justify-content:center;margin-top:14px" href="https://wa.me/243825001290?text=${message}" target="_blank">Commander sur WhatsApp →</a>
      </div>
    </div>`;
