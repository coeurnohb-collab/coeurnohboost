...
async function submitRecharge() {
  ...
  try {
    ...
    if (payMethod === 'mobile') {
      ...
      var cinetpayDebugReason = cinetpayData.reason || null;
    }
    ...
    // Diagnostic : si CinetPay a ete tente sans succes, on garde la raison exacte
    // dans Firestore pour la retrouver facilement depuis mobile (pas de logs Vercel a fouiller)
    if (typeof cinetpayDebugReason !== 'undefined' && cinetpayDebugReason) {
      manualRequestData.debugReason = cinetpayDebugReason;
    }
    await db.collection('topup_requests').add(manualRequestData);
