/* =========================================================
   FIREBASE CONFIG — Projet Coeurnoh Boost
   ========================================================= */
...
try {
  ...
  console.error("🔴 Firebase non disponible (connexion/commandes désactivées) :", e.message);
}

// UID Admin par défaut (remplace par ton propre UID Firebase une fois connecté)
const ADMIN_UID = "8BqWONj07hVZePHe2DrkHWYRjse2";

/* =========================================================
   SYSTEME DE TRADUCTION & DÉTECTION DE LANGUE
   ========================================================= */
function detectLang() {
...
function showNotification(message){
  ...
  setTimeout(()=>{ toast.classList.add('toast-out'); setTimeout(()=>toast.remove(), 400); }, 3200);
}
...
const DEFAULT_CATALOG = {
  TikTok: [
    ...
    {name:"Pack Tout-en-un", desc:"Followers + Vues + Likes combinés", tiers:[{label:"Standard",price1k:3.0,min:100},{label:"Premium ⭐",price1k:7.8,min:50},{label:"VIP 👑",price1k:12.5,min:50}]},
  ],
  Instagram: [
    ...
    {name:"Commentaires", desc:"Commentaires positifs variés", tiers:[{label:"Standard",price1k:5,min:10},{label:"Premium ⭐",price1k:11,min:10},{label:"VIP 👑",price1k:17.6,min:10}]},
    {name:"Pack Tout-en-un", desc:"Followers + Vues + Likes combinés", tiers:[{label:"Standard",price1k:3.7,min:100},{label:"Premium ⭐",price1k:8.7,min:50},{label:"VIP 👑",price1k:13.9,min:50}]},
  ],
  YouTube: [
    ...
    {name:"Pack Tout-en-un", desc:"Abonnés + Vues + Likes combinés", tiers:[{label:"Standard",price1k:7.2,min:100},{label:"Premium ⭐",price1k:17,min:50},{label:"VIP 👑",price1k:27.2,min:50}]},
  ],
  Facebook: [
    ...
    {name:"Commentaires", desc:"Commentaires positifs variés", tiers:[{label:"Standard",price1k:5,min:10},{label:"Premium ⭐",price1k:11,min:10},{label:"VIP 👑",price1k:17.6,min:10}]},
    {name:"Partages", desc:"Amplifie la portée organique", tiers:[{label:"Standard",price1k:1.5,min:50},{label:"Premium ⭐",price1k:3.5,min:50},{label:"VIP 👑",price1k:5.6,min:50}]},
    {name:"Pack Tout-en-un", desc:"Followers + Vues + Likes combinés", tiers:[{label:"Standard",price1k:3.4,min:100},{label:"Premium ⭐",price1k:8.9,min:50},{label:"VIP 👑",price1k:14.3,min:50}]},
  ],
  Spotify: [
  ...
  SoundCloud: [
    ...
    {name:"Followers", desc:"Croissance progressive", tiers:[{label:"Standard",price1k:2.5,min:100},{label:"Premium ⭐",price1k:6,min:50},{label:"VIP 👑",price1k:9.6,min:50}]},
    {name:"Likes", desc:"Boost d'engagement", tiers:[{label:"Standard",price1k:1,min:100},{label:"Premium ⭐",price1k:2.5,min:50},{label:"VIP 👑",price1k:4,min:100}]},
  ],
  "Apple Music": [
...
const MONETIZATION_PACKS = [
  {name:"Pack Monétisation YouTube", desc:"4000h de watch time + 1000 abonnés — seuil Partner Program", criteria:"Critères YouTube : 1 000 abonnés + 4 000h de visionnage sur 12 mois", price:150},
...
const TESTIMONIALS = [
  {name:"Serge K.", text:"Super service pour mon compte TikTok, livraison rapide !", rating:5},
  {name:"Amina M.", text:"Pack monétisation YouTube parfait, validation rapide.", rating:5},
];

/* =========================================================
   RESEAUX MOBILE MONEY & MONNAIES
   ========================================================= */
...
const PACKAGE_PLANS = [
  ...
  {name:"Business", tagline:"Pour les marques", price:120, includes:["4 plateformes au choix","10 000 unités qualité VIP","Support dédié 7j/7","2 packs combinés offerts","Rapport mensuel"]},
];
...
function applyTranslations(){
  ...
  renderTabs(); renderServices(); renderMonetization(); renderCombos(); renderTestimonials(); renderPackagePlans();
  renderAppNavLabels();
}
...
function toggleLangMenu(){ const langMenu = document.getElementById('lang-menu'); if(langMenu) langMenu.classList.toggle('hidden'); }
...
function switchAppView(view){
  ...
  if(view==='commande'){ renderTabs(); const svcVal = document.getElementById('d-service'); if(svcVal && !svcVal.value) populateDashServiceSelect(currentTab, null); }
...
function openAuth(mode){ authMode = mode || 'login'; updateAuthModalMode(); document.getElementById('auth-modal')?.classList.remove('hidden'); }
...
function updateAuthModalMode(){
  ...
  const titleEl = document.getElementById('auth-title'); if(titleEl) titleEl.textContent = isReg ? 'Créer un compte' : 'Se connecter';
  const nameField = document.getElementById('auth-name-field'); if(nameField) nameField.classList.toggle('hidden', !isReg);
  const submitBtn = document.getElementById('auth-submit'); if(submitBtn) submitBtn.textContent = isReg ? 'Créer mon compte' : 'Se connecter';
  const switchTxt = document.getElementById('auth-switch-text'); if(switchTxt) switchTxt.textContent = isReg ? 'Déjà un compte ?' : 'Pas encore de compte ?';
  const switchBtn = document.getElementById('auth-switch-btn'); if(switchBtn) switchBtn.textContent = isReg ? 'Se connecter' : "S'inscrire";
...
async function submitAuth(){
  ...
  if(!fbReady){ showAuthError("Connexion au service indisponible pour le moment. Vérifie ta connexion."); return; }
  ...
  try {
    if(authMode==='register'){
      ...
      await db.collection('users').doc(cred.user.uid).set({
        ...
        balance: 0,
        lang: currentLang,
        referredBy: getReferralCodeFromURL() || null,
...
async function signInWithGoogle(){
  if(!fbReady){ showAuthError("Connexion au service indisponible pour le moment."); return; }
...
function logout(){ if(fbReady && auth) auth.signOut(); exitApp(); }
...
function copyReferralLink(){
  ...
  navigator.clipboard.writeText(link).then(()=>{
    const copyMsg = document.getElementById('referral-copy-msg');
    if(copyMsg) copyMsg.textContent = "✅ Lien copié !";
  }).catch(()=>{
    const copyMsg = document.getElementById('referral-copy-msg');
    if(copyMsg) copyMsg.textContent = link;
  });
}
...
async function loadReferralStats(){
  ...
  } catch(e){ console.warn("Stats parrainage indisponibles :", e.message); }
}
  ...
  auth.onAuthStateChanged(async (user) => {
    if(user){
      try {
        ...
        const dashUsername = document.getElementById('dash-username'); if(dashUsername) dashUsername.textContent = currentUser.name;
        const profileName = document.getElementById('profile-name'); if(profileName) profileName.textContent = currentUser.name;
        const profileEmail = document.getElementById('profile-email'); if(profileEmail) profileEmail.textContent = currentUser.email;
        const profileSince = document.getElementById('profile-since'); if(profileSince) profileSince.textContent = new Date(currentUser.createdAt || Date.now()).toLocaleDateString('fr-FR');
...
function onCountryChange(){
  const select = document.getElementById('country-select');
  if(select) selectedCountry = select.value;
  renderMobileNetworks();
  updateLocalConversion();
}
...
function updateLocalConversion(){
  ...
  const amount = parseFloat(depositInput ? depositInput.value : 0) || 0;
  el.textContent = amount > 0 ? convertToLocal(amount) + " (taux indicatif)" : '';
}
...
async function redeemLoyaltyPoints(){
  ...
  if(points < 100){ if(msgEl){ msgEl.textContent = "Il te faut au moins 100 points (tu en as " + points + ")."; msgEl.style.color = 'var(--red)'; } return; }
...
async function callPaymentGateway({amount, phone, network}){
  if(network === 'card'){
    return {success:false, reason:"Passerelle carte bancaire non configurée."};
  }
  return {success:false, reason:"Demande enregistrée pour traitement manuel."};
}
...
async function submitDeposit(){
  ...
  if(amount <= 0 || (!isCard && !phone)){ alert("Renseigne un montant et des informations valides."); return; }
  ...
  if(result.success){
    ...
    showNotification("⏳ Demande enregistrée. Contacte le support si besoin.");
  }
}
...
function getSelectedTier(){
  ...
  const svc = (CATALOG[chosen.platform]||[]).find(s => s.name === chosen.name);
  if(!svc) return null;
  const tierIndex = parseInt(qualitySelect ? qualitySelect.value : 0) || 0;
...
async function placeOrder(){
  ...
  if(!currentUser){ openAuth('login'); return; }
  const sel = getSelectedTier();
  if(!sel){ if(msgEl){ msgEl.textContent = "Choisis un service."; msgEl.style.color='var(--red)'; } return; }
  ...
  if(!rawLink.trim()){ if(msgEl){ msgEl.textContent = "Ajoute le lien du profil/publication."; msgEl.style.color='var(--red)'; } return; }
  if(!isValidLink(link)){ if(msgEl){ msgEl.textContent = "Lien invalide (doit commencer par http:// ou https://)."; msgEl.style.color='var(--red)'; } return; }
  if(cost > (currentUser.balance||0)){ if(msgEl){ msgEl.textContent = "Solde insuffisant — recharge ton portefeuille."; msgEl.style.color='var(--red)'; } return; }
  ...
  try {
    ...
    if(msgEl){ msgEl.textContent = "Erreur lors du traitement de la commande."; msgEl.style.color='var(--red)'; }
  }
}
  ...
  ordersUnsubscribe = db.collection('orders')
    ...
    .onSnapshot(snap => {
      ...
      if(snap.empty){
        ...
        const countEl = document.getElementById('dash-order-count'); if(countEl) countEl.textContent = '0';
        return;
      }
      const countEl = document.getElementById('dash-order-count'); if(countEl) countEl.textContent = snap.size;
      tbody.innerHTML = snap.docs.map(d => {
        ...
        return `<tr><td>${date}</td><td>${o.platform} — ${o.service}</td><td>${o.qty}</td><td>${(o.amount||0).toFixed(2)}$</td><td><span class="status ${statusClass}">${statusLabel}</span></td></tr>`;
      }).join('');
    }, err => {
      console.warn("Suivi temps réel indisponible :", err.message);
    });
}
...
async function loadCatalogOverrides(){
  ...
  } catch(e){ console.warn("Pas de tarifs personnalisés :", e.message); }
}
