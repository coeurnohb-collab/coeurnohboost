...
/* Fil de publications sur l'Accueil (photos/produits/livres), style Facebook :
   simple, sans recherche ni filtres — juste les publications recentes. */
async function loadHomeFeed() {
  ...
  try {
    ...
    if (currentUser) {
      ...
      ordersSnap.docs.forEach(doc => purchasedSet.add(doc.data().pubId));
    }

    feedEl.innerHTML = items.map(item =>
      renderShopCard(item, likedMap[item.id], purchasedSet.has(item.id))
    ).join('');
  } catch (e) {
