...
async function publishShopItem() {
  ...
  const bookFile = document.getElementById('shop-file').files[0];
  ...
  if (!imageFile) {
    showShopFormError("Merci d'ajouter une photo.");
    return;
  }
  if (type === 'book' && !bookFile) {
    showShopFormError("Merci d'ajouter le fichier PDF du livre.");
    return;
  }

  const progressEl = document.getElementById('shop-upload-progress');
  progressEl.classList.remove('hidden');

  try {
    const itemId = db.collection('publications').doc().id;

    // Upload de la photo
    const imageRef = storage.ref(`publications/${itemId}/image_${imageFile.name}`);
    await imageRef.put(imageFile);
    const imageUrl = await imageRef.getDownloadURL();

    // Upload du fichier PDF (livres uniquement)
    let fileUrl = null;
    if (type === 'book') {
      const fileRef = storage.ref(`publications/${itemId}/file_${bookFile.name}`);
      await fileRef.put(bookFile);
      fileUrl = await fileRef.getDownloadURL();
    }

    await db.collection('publications').doc(itemId).set({
      type,
      title,
      description,
      price,
