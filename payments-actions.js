...
//   { idToken, action: 'site_domain_purchase', desiredDomain } -- demande
...
const SITE_DOMAIN_PRICE_YEAR = 15;      // en $, par an -- valeur de repli si l'admin n'a rien configuré dans pricing/site (doit rester identique à SITE_DOMAIN_PRICE_YEAR dans script.js)
...
async function getSitePricingConfig() {
  ...
  try {
    ...
    return {
      ...
      domainPlans: (Array.isArray(data.domainPlans) && data.domainPlans.length > 0) ? data.domainPlans : fallback.domainPlans
    };
  } catch (e) {
    return fallback;
  }
...
async function purchaseSiteDomain(uid, body) {
  ...
  // Le prix et la duree viennent TOUJOURS de la config admin cote serveur
  // (jamais du client) -- seul l'identifiant de la formule choisie (planId)
  // est envoye par le client, pour eviter qu'une personne malveillante
  // n'envoie elle-meme un prix de 0.
  const pricingConfig = await getSitePricingConfig();
  const plan = pricingConfig.domainPlans.find(p => p.id === body.planId) || pricingConfig.domainPlans[0];
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    transaction.set(reqRef, {
      uid, desiredDomain, status: 'pending', pricePaid: price, planId: plan.id, planLabel: plan.label || null, months,
      createdAt: new Date().toISOString()
    });
    transaction.set(db.collection('wallet_transactions').doc(), buildWalletTxEntry({
