...
// Envoie une VRAIE alerte push sur le telephone du client (meme app fermee),
// en plus de la notification deja ecrite dans Firestore. Echec silencieux
// si le client n'a jamais autorise les notifications (pas grave, la notif
// reste visible dans l'app via la page Notifications).
    ...
    if (status === 'completed') {
      ...
      await db.runTransaction(async (transaction) => {
        ...
        transaction.set(db.collection('wallet_transactions').doc(), buildWalletTxEntry({
          uid: requestData.uid, type: 'topup_mobile_money', amount: amountUSD, balanceAfter: newBalance,
          description: 'Recharge par Mobile Money', relatedId: reference
        }));
      ...
      await sendPushNotification(requestData.uid, 'Solde rechargé 💰', `Ton compte a été crédité de ${amountUSD.toFixed(2)}$.`);
