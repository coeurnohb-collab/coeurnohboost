...
export default async function handler(req, res) {
  // Securite : CRON_SECRET est maintenant OBLIGATOIRE (ajoute-le dans les
  // variables d'environnement Vercel puis dans vercel.json en en-tete du
  // cron -- voir le fichier vercel.json livre avec ce chantier). Avant, en
  // l'absence de cette variable, N'IMPORTE QUI pouvait appeler cette URL
  // publique pour declencher des envois WhatsApp/email a volonte.
  const cronSecret = process.env.CRON_SECRET;
  if (!cronSecret) {
    console.error('[check-mtp-balance] CRON_SECRET manquant : appel refuse par securite.');
  ...
  try {
    ...
    const message = `⚠️ Coeurnoh Universe : ton solde MoreThanPanel est bas (${balance.toFixed(2)} ${currency}). Recharge-le vite pour eviter que les commandes clients echouent.`;
